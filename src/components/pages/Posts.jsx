...
import { 
  ...
  X, ImageIcon, Video, Send, Upload, Sparkles,
  ChevronRight, Eye, Heart, ArrowUp, Grid3X3, Home,
  Search, Plus, User, Bell
} from 'lucide-react';
...
const ShinyText = ({ children, className = "" }) => {
  return (
    <span className={`inline-block bg-gradient-to-r from-white via-blue-300 to-white bg-clip-text text-transparent animate-shimmer bg-[length:200%_100%] ${className}`}>
      {children}
    </span>
  );
};
...
// Stories Component - NEW
const Stories = () => {
  const stories = [
    { id: 1, name: 'Your Story', avatar: 'https://i.pravatar.cc/150?img=7', isYou: true },
    { id: 2, name: 'Sarah', avatar: 'https://i.pravatar.cc/150?img=5', isLive: true },
    { id: 3, name: 'Mike', avatar: 'https://i.pravatar.cc/150?img=1' },
    { id: 4, name: 'Emma', avatar: 'https://i.pravatar.cc/150?img=2' },
    { id: 5, name: 'Alex', avatar: 'https://i.pravatar.cc/150?img=8' },
    { id: 6, name: 'Maya', avatar: 'https://i.pravatar.cc/150?img=9' },
  ];

  return (
    <div className="bg-zinc-900/50 backdrop-blur-xl rounded-2xl p-4 border border-zinc-800/50 mb-6">
            ...
            <div key={story.id} className="flex flex-col items-center space-y-2">
              <div className={`relative p-0.5 rounded-full ${
                ...
                <div className="bg-zinc-900 p-1 rounded-full">
                  <Avatar className="w-16 h-16 border-2 border-zinc-900">
                    <AvatarImage src={story.avatar} />
                    <AvatarFallback>{story.name[0]}</AvatarFallback>
                  </Avatar>
                </div>
                {story.isYou && (
                  <div className="absolute -bottom-1 left-1/2 transform -translate-x-1/2 bg-blue-500 rounded-full p-1 border-2 border-zinc-900">
                    <Plus size={12} className="text-white" />
                  </div>
                )}
              </div>
              <p className="text-xs text-zinc-400 max-w-[80px] truncate">
...
const LeftSidebar = () => {
  ...
  const suggestions = [
    { name: 'Webulsylist', location: 'Elk Grove, California', avatar: 'https://i.pravatar.cc/150?img=12' },
    { name: 'Anghelina', location: 'Sibiu, Romania', avatar: 'https://i.pravatar.cc/150?img=13' },
    { name: 'Male Designer', location: 'Ukraine', avatar: 'https://i.pravatar.cc/150?img=14' },
    { name: 'Vera Cherry', location: 'Bremen, Germany', avatar: 'https://i.pravatar.cc/150?img=15' },
    { name: 'Josh e-Sport', location: 'Elk Grove, California', avatar: 'https://i.pravatar.cc/150?img=16' },
  ];
        ...
        <CardContent className="p-6">
          ...
          <div className="grid grid-cols-3 gap-4 text-center mb-4">
            <div>
              ...
              <p className="text-xs text-zinc-400">Lite</p>
            </div>
            <div>
        ...
        <CardContent className="p-4">
          <div className="space-y-4">
            {trending.map((trend, index) => (
              <div key={trend.hashtag} className="flex items-center justify-between p-3 rounded-lg bg-zinc-800/30 hover:bg-zinc-800/50 cursor-pointer transition-colors">
                <div className="flex items-center space-x-3">
                  <div className="w-8 h-8 bg-gradient-to-br from-blue-500 to-purple-500 rounded-full flex items-center justify-center">
                    <span className="text-xs font-bold text-white">#{index + 1}</span>
                  </div>
                  <div>
                    <p className="text-sm font-medium text-white">{trend.hashtag}</p>
      ...
      <Card className="bg-zinc-900/50 backdrop-blur-xl border-zinc-800/50">
        ...
        <CardContent className="p-4">
          ...
          <div className="bg-zinc-800/30 rounded-lg p-4">
            <p className="text-sm text-blue-300 font-medium mb-2">Active now on your profile</p>
            <p className="text-xs text-zinc-400">
              Apply for a feature following the link in our bio and we will publish your photos in our account: @travelsfever
            </p>
          </div>
        </CardContent>
      </Card>
...
const MultiImageGrid = ({ images, onImageClick }) => {
  ...
  const renderImages = () => {
    ...
    if (imageCount === 1) {
      return (
        <div className="relative group cursor-pointer" onClick={() => onImageClick(images[0])}>
          <img 
            src={images[0]} 
            alt="Post content" 
    ...
    return (
      <div className={`grid ${getGridClass(imageCount)} rounded-xl overflow-hidden`}>
...
const MediaViewerSheet = ({ mediaUrl, mediaType, trigger, isMultiImage = false, images = [] }) => {
  ...
  if (isMultiImage && images.length > 0) {
    return (
      <Sheet>
        ...
        <SheetContent  style={{zIndex:999999999}} side="bottom" className="h-[90vh] bg-black/25 backdrop-blur-xl border-white/10">
          <SheetHeader>
            <SheetTitle className="text-white flex items-center gap-2">
              <ShinyText>Media Preview</ShinyText>
              <Badge variant="outline" className="border-blue-300/50 text-blue-300 bg-blue-500/10">
  ...
  return (
    <Sheet>
      ...
      <SheetContent side="bottom" className="h-[95vh] bg-black/95 backdrop-blur-xl border-white/10">
        <SheetHeader>
        ...
        <div className="flex items-center justify-center h-full">
          {mediaType === 'image' ? (
            <img src={mediaUrl} alt="Full preview" className="max-h-full max-w-full object-contain rounded-lg" />
          ) : (
...
const CommentDialog = ({ comments, postAuthor }) => {
  ...
  const [commentList, setCommentList] = useState([
    { id: 1, author: 'Alex Chen', avatar: 'https://i.pravatar.cc/150?img=8', text: 'This is amazing! Great work!', timestamp: '1h ago' },
    { id: 2, author: 'Maya Patel', avatar: 'https://i.pravatar.cc/150?img=9', text: 'Love the attention to detail here.', timestamp: '2h ago' }
  ]);

  const handleAddComment = () => {
    if (!newComment.trim()) return;
    setCommentList([
      { id: commentList.length + 1, author: 'You', avatar: 'https://i.pravatar.cc/150?img=7', text: newComment, timestamp: 'Just now' },
      ...commentList
    ]);
    setNewComment('');
  };
  ...
  return (
    <Dialog>
      <DialogTrigger asChild>
        <Button variant="ghost" size="sm" className="gap-2 text-zinc-400 hover:text-blue-400 hover:bg-blue-500/10 transition-all">
            ...
            {commentList.map(comment => (
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-zinc-500 mt-1 ml-3">{comment.timestamp}</p>
                </div>
              </motion.div>
            ))}
          </div>
...
const ShareSheet = () => {
  return (
    <Sheet>
      <SheetTrigger asChild>
        <Button variant="ghost" size="sm" className="gap-2 text-zinc-400 hover:text-purple-400 hover:bg-purple-500/10 transition-all">
          <Share2 size={18} />
      ...
      <SheetContent className="bg-zinc-950 border-zinc-800 text-white">
        ...
        <div className="space-y-3 mt-6">
          {['Copy Link', 'Share to Twitter', 'Share to LinkedIn', 'Email'].map((option) => (
            <Button
              key={option}
              variant="outline"
...
const PostCard = ({ post }) => {
  ...
  const hasMultipleImages = post.type === 'image' && Array.isArray(post.mediaUrl) && post.mediaUrl.length > 1;
              ...
              trigger={
                ...
                >
                  {post.type === 'image' ? (
                    ...
                    <video src={post.mediaUrl} className="w-full rounded-xl" controls />
                  )}
                </motion.div>
              }
            />
          ...
          {files.length > 0 && (
            ...
            >
              ...
              {files.length === 1 ? (
                <div className="relative rounded-xl overflow-hidden">
                  {fileType === 'image' ? (
                    <img src={files[0]} alt="Preview" className="max-h-80 w-full object-contain rounded-xl" />
                  ) : (
                    <video src={files[0]} controls className="max-h-80 w-full rounded-xl" />
                  )}
                  <Button
  ...
  const initialPosts = [
    {
      ...
      caption: 'Project development progress shots! Multiple angles of our new interface design. 💡 #devlife #uiux',
      likes: 128,
      comments: 12,
    },
    {
      ...
      caption: 'Team brainstorming session today. Before and after shots of our workspace setup!',
      likes: 256,
      comments: 23,
    },
    {
  ...
  const handleCreatePost = (postData) => {
    if (postData && (postData.files?.length || postData.caption)) {
      const newPost = {
        ...
        comments: 0,
        type: postData.type,
        mediaUrl: postData.files?.length > 1 ? postData.files : (postData.files?.[0] || null),
      };
