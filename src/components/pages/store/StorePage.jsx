 ...
 * Browse and purchase virtual items with SF Coins, Crystals, or Event Tokens
 */
...
const StorePage = () => {
  ...
  const handlePurchase = async (product) => {
    if (!canAfford(product)) {
      toast.error(`Insufficient ${product.currency_type.replace('_', ' ')}`);
      return;
    }
  ...
  const canAfford = (product) => {
    ...
    switch (product.currency_type) {
      ...
      case 'event_tokens': return wallet.event_tokens >= product.price;
  ...
  const getBalance = (currencyType) => {
    ...
    switch (currencyType) {
      ...
      case 'event_tokens': return wallet.event_tokens;
      default: return 0;
    }
  };
    ...
    <div className="min-h-screen pb-20 px-4 md:px-6 lg:px-8 max-w-7xl mx-auto">
        ...
        <div className="flex items-center gap-2">
          {wallet && (
            <>
              ...
              <WalletBadge icon={Ticket} value={wallet.event_tokens} color="text-cyan-400" bg="bg-cyan-500/10" />
            </>
          )}
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {filteredProducts.map((product, index) => (
          <ProductCard key={product.id} product={product} canAfford={canAfford(product)} onPurchase={() => setSelectedProduct(product)} purchasing={purchasingId === product.id} index={index} />
        ))}
      </motion.div>
