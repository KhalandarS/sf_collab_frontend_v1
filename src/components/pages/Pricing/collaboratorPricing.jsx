  ...
  const plans = [
    {
      ...
      buttonColor: "bg-gradient-to-r from-purple-600 to-black text-gray-200 hover:cursor-pointer",
    },
    {
      name: "Builder Plan",
      price: "$9.99/month",
      ...
      color: "border-purple-500 bg-[#111111]",
      buttonColor: "bg-gradient-to-r from-purple-600 to-black text-gray-200 hover:cursor-pointer",
    },
    {
      name: "Team Lead Plan",
  ...
  return (
    ...
    >
      <div className="mx-auto max-w-7xl px-4">
        ...
        >
          <h2 className="text-4xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-gray-700 to-gray-200">
          ...
          {plans.map((plan, index) => (
            <motion.div
              ...
              className={`
                ...
                ${plan.recommended ? 'ring-2 ring-purple-700 ring-offset-2' : ''}
                ${plan.color}
              `}
            >
              {plan.recommended && (
                <div className="absolute -top-3 left-1/2 transform -translate-x-1/2">
                  <span className="bg-gradient-to-r from-purple-700 to-black text-white px-4 py-1 rounded-full text-sm font-semibold">
                    MOST POPULAR
                  </span>
                </div>
              )}
              ...
              <div className="mb-6">
                <h3 className="text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-gray-600 to-gray-300">{plan.name}</h3>
                <div className="mt-2 flex items-baseline">
                  <span className="text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-gray-600 to-gray-300">{plan.price}</span>
                  {plan.price !== "$0" && (
                    <span className="ml-2 text-transparent bg-clip-text bg-gradient-to-r from-gray-600 to-gray-300"></span>
                  )}
                </div>
                <p className="mt-2 text-sm text-gray-400">{plan.description}</p>
                <div className="mt-3 inline-block bg-gradient-to-r from-purple-700 to-black px-3 py-1 rounded-full">
                  <span className="text-xs font-medium text-gray-200">{plan.role}</span>
                </div>
              </div>

              {/* Virtual Currency Benefits */}
              <div className="mb-6 p-4 bg-gradient-to-br from-purple-700 to-black rounded-xl">
                <h4 className="font-semibold text-gray-200 mb-2">Virtual Currency Benefits</h4>
                <div className="grid grid-cols-3 gap-2">
                  <div className="text-center">
                    <div className="text-lg font-bold text-gray-200">{plan.virtualCurrency.xpMultiplier}</div>
                    <div className="text-xs text-gray-200">XP Multiplier</div>
                  </div>
                  <div className="text-center">
                    <div className="text-lg font-bold text-gray-200">{plan.virtualCurrency.exchangeBonus}</div>
                    <div className="text-xs text-gray-200">Exchange Bonus</div>
                  </div>
                  <div className="text-center">
                    <div className="text-lg font-bold text-gray-200">{plan.virtualCurrency.monthlyCoins}</div>
                    <div className="text-xs text-gray-200">Monthly</div>
                  </div>
                </div>
              </div>
              ...
              <div className="mb-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {Array.isArray(plan.targetRoles) ? plan.targetRoles.map((role, idx) => (
                    <span key={idx} className="px-2 py-1 bg-gradient-to-r from-purple-700 to-black text-gray-200 rounded-full text-xs">
                      {role}
                    </span>
                  )) : (
        ...
        >
          <h3 className="text-2xl font-bold mb-6 text-gray-300">Role-Specific Add-on Bundles &nbsp; </h3>
